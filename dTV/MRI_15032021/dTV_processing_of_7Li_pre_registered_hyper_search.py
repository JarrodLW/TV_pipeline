...
outputfile = save_dir + '/dTV_7Li_15032021_' + str(n) + '_pre_registered_hyper_search_gamma_' + '{:.1e}'.format(gamma) + '.json'
    ...
    for dict_key in sinfos.keys():
        ...
        for eta in etas:
            ...
            if 'alpha=' + '{:.1e}'.format(alpha) not in d['measurement=' + str(i)]['output_size=' + str(sinfo.shape[0])].keys():
                ...
                d['measurement=' + str(i)]['output_size=' + str(sinfo.shape[0])]['eta=' + '{:.1e}'.format(eta)]['recon'] = recon.tolist()
                d['measurement=' + str(i)]['output_size=' + str(sinfo.shape[0])]['eta=' + '{:.1e}'.format(eta)]['affine_params'] = palm.x[1].asarray().tolist()
                d['measurement=' + str(i)]['output_size=' + str(sinfo.shape[0])]['eta=' + '{:.1e}'.format(eta)]['fourier_diff'] = [
