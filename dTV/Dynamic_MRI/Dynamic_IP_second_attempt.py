...
if phantom_test:
    ...
    complex_im = image_space.element([phantom, image_space[1].zero()])
    ...
    for i in range(frame_num):
        noise = 0.1*np.random.normal(loc=0.0, scale=0.5, size=synth_data.shape) \
                                + 0.1*1j*np.random.normal(loc=0.0, scale=0.5, size=synth_data.shape)
...
if Li_test:
    ...
    f_coeff_list = []
    naive_recons = np.zeros((frame_num, 32, 32), dtype='complex')

    for i in range(3, 3+frame_num):
        ...
        naive_recons[i-3] = np.fft.ifftshift(np.fft.ifft2(f_coeffs_unpacked))

    data_stack = np.asarray(f_coeff_list)
    ...
    im_array = np.zeros((frame_num, 32, 32), dtype='complex')

    for i in range(frame_num):
        im_array[i] = naive_recon_fully_averaged
    ...
    for i in range(frame_num):
        ...
        if clean_first_and_last_images:
            if i == 0:
                ...
                noise = 0.2 * np.random.normal(loc=0.0, scale=0.5, size=[height, width]) \
                        + 0.2 * 1j * np.random.normal(loc=0.0, scale=0.5, size=[height, width])
        ...
        # noise = 0.3 * np.random.normal(loc=0.0, scale=0.5, size=[height, width]) \
        #         + 0.3 * 1j * np.random.normal(loc=0.0, scale=0.5, size=[height, width])
        synth_data_with_noise = synth_data + noise
        data_stack[i] = synth_data_with_noise
        naive_recons[i] = np.fft.ifftshift(np.fft.ifft2(synth_data_with_noise))
...
spatial_reg = alpha * odl.solvers.GroupL1Norm(spatial_grad_codomain)
...
op = odl.BroadcastOperator(f_op, spatial_G_stacked, temporal_G_stacked)
